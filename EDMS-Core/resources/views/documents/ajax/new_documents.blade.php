        ...
        <div class="panel-body">
          ...
          <table class="table table-bordered table-striped table-hover">
            ...
            <tbody>
              ...
              <a class="get myTooltip btn btn-default btn-md" href="{{route('documents.show', $document->id)}}">
                <i class="fa fa-eye"></i> 
                    <span class="tooltiptext">{{App\Global_var::getLangString('View_Details', $language_strings)}}</span>
              </a> 
               @endif
                @if($currentUser->isGranted('documents.approveNewDocument'))
                  <a class="get myTooltip btn btn-md btn-success" href="{{route('documents.approveNewDocument', [$document->id, 1])}}" nextUrl="{{route('documents.newDocuments')}}"><i class="fa fa-check"></i> 
                  <span class="tooltiptext">{{App\Global_var::getLangString('Approve', $language_strings)}}</span>
                  </a>
                    <a class="get myTooltip btn btn-md btn-danger" href="{{route('documents.approveNewDocument', [$document->id, 0])}}" nextUrl="{{route('documents.newDocuments')}}"><i class="fa fa-close"></i>
                    <span class="tooltiptext">{{App\Global_var::getLangString('Reject', $language_strings)}}</span>
                    </a>
               @endif
