...
public function heavy_crime_ksi_mezgebat(){
      ...
      //return $this->hasMany('App\ksi_mezgeb', 'zoneId', 'id')->where('crimeWeight', '=', 'Medium_And_Havy')->orderBy('id', 'desc');
    }
/* public function leading_investigation_of_heavy_crimes(){
 ...
 public function human_rights(){
      $human_rights=\App\human_right::where('zoneId', '=', $this->id);
      return $human_rights;
    }
  public function complains_by_customerData(){
      ...
      $data_array=array(
                        ...
                        'Rank_Satisfaction'=>$Rank_Satisfaction, 
                        );
...
$effectiveness_count=0;
  ...
  public function free_law_support_for_organizations(){
      ...
      $kttln_dgafn_abyate_eyo=$this->kttln_dgafn_abyate_eyo();/*\App\kttln_dgafn_abyate_eyo::where('zoneId', '=', $this->id);*/
 ...
 public function Witnessed_Count(){
      $witness_follow_ups=$this->witness_follow_up();//\App\witness_follow_up::where('zoneId', '=', $this->id)->get();
      $male_count=0; $female_count=0;
      foreach ($witness_follow_ups as $follow_up) {
...
public function Payment_Of_Allowance(){
 ...
 public function Appointment_Count(){
    ...
    public function ksi_mezgebat(){
      ...
      //return $this->hasMany('App\ksi_mezgeb', 'zoneId', 'id')->orderBy('id', 'desc');
    }
...
$ksi_mezgeb_brkitat_Array=array();
      ...
      foreach ($ksi_mezgeb_brkitat as $ksi_mezgeb_brki) {
        ...
        if($ksi_mezgeb==null)
          continue;
        if($currentUser->isAdmin() || $currentUser->isRegion()){
        ...
        /*if($currentUser->isAdmin() && $ksi_mezgeb_brki->appealedToBrkiId!=null)
            continue;*/

        if($currentUser->isAdmin() && $ksi_mezgeb_brki->isDirect() && $ksi_mezgeb_brki->ksi_mezgeb->created_a_brki!="Zone" && $ksi_mezgeb_brki->ksi_mezgeb->created_a_brki!="Wereda")
            continue;

          

          if($ksi_mezgeb_brki->ksi_mezgeb==null || $ksi_mezgeb_brki->ksi_mezgeb->zoneId!=$this->id || (!$currentUser->isAdmin() && $ksi_mezgeb_brki->isDirect() && $ksi_mezgeb_brki->ksi_mezgeb->created_a_brki!="Zone"))
            continue;
...
if(!$ksi_mezgeb_brki->isNewDocument() && !$ksi_mezgeb_brki->isRoundedDocument())
  continue;
   ...
   /*-------SERVICES-------------*/
      ...
      $entities=[];       
       $currentUser=\Auth::guard('web')->user();
     if(!$currentUser->isZone() && !$currentUser->isAdmin() && !$currentUser->isZone_Admin())
        return $entities;

       $glgalot_ntsadgaf_hgi=$currentUser->glgalot_ntsadgaf_hgi();
      foreach ($glgalot_ntsadgaf_hgi as $entity) {
        if($currentUser->isAdmin() && $entity->created_a_brki!="Zone" && $entity->created_a_brki!="Wereda")
            continue;

          if($entity->zoneId!=$this->id || (!$currentUser->isAdmin() && $entity->created_a_brki!="Zone"))
            continue;
