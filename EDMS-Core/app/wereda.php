...
public function heavy_crime_ksi_mezgebat(){
      ...
      //return $this->hasMany('App\ksi_mezgeb', 'weredaId', 'id')->where('crimeWeight', '=', 'Medium_And_Havy')->orderBy('id', 'desc');
    }
 ...
 /*public function human_rights(){
      $human_rights=\App\human_right::where('weredaId', '=', $this->id);
      return $human_rights;
    }*/
...
$effectiveness_count=0;
  ...
  public function free_law_support_for_organizations(){
      ...
      $kttln_dgafn_abyate_eyo=$this->kttln_dgafn_abyate_eyo();/*\App\kttln_dgafn_abyate_eyo::where('weredaId', '=', $this->id);*/
 ...
 public function Witnessed_Count(){
      $witness_follow_ups=$this->witness_follow_up();//\App\witness_follow_up::where('weredaId', '=', $this->id)->get();
      $male_count=0; $female_count=0;
      foreach ($witness_follow_ups as $follow_up) {
 ...
 public function Appointment_Count(){
    ...
    public function ksi_mezgebat(){
      ...
      //return $this->hasMany('App\ksi_mezgeb', 'weredaId', 'id')->orderBy('id', 'desc');
    }
    ...
    $currentUser=\Auth::guard('web')->user();
      ...
      foreach ($ksi_mezgeb_brkitat as $ksi_mezgeb_brki) {
        if((/*$currentUser->isZone_Admin() || */$currentUser->isAdmin()) && $ksi_mezgeb_brki->isAppealedToNextBrki())
            continue;

          if(!$ksi_mezgeb_brki->isDirect())
            continue;

          if($ksi_mezgeb_brki->ksi_mezgeb==null || $ksi_mezgeb_brki->ksi_mezgeb->weredaId!=$this->id || (/*!$currentUser->isAdmin() && !$currentUser->isZone_Admin() && */$ksi_mezgeb_brki->isDirect() && $ksi_mezgeb_brki->ksi_mezgeb->created_a_brki!="Wereda"))
            continue;
  ...
  continue;
   ...
   /*-------SERVICES-------------*/
    ...
    $currentUser=\Auth::guard('web')->user();
      ...
      if(!$currentUser->isWereda() && !$currentUser->isAdmin() && !$currentUser->isZone_Admin())
        return $entities;
      
      foreach ($glgalot_ntsadgaf_hgi as $entity) {
          if($entity->weredaId!=$this->id || (/*!$currentUser->isAdmin() && !$currentUser->isZone_Admin() && */$entity->created_a_brki!="Wereda"))
            continue;
